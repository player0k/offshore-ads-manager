...
if ( ! class_exists( 'FW_Option_Type_Multi_Select_Extended' ) ):

	/**
	 * Select multiple choices from different sources: posts, taxonomies, users or custom array
	 */
	class FW_Option_Type_Multi_Select_Extended extends FW_Option_Type {
		/**
		 * @internal
		 */
		...
		public static function _admin_action_get_ajax_response() {
			...
			$term = FW_Request::POST( 'data/term' );
			$posts = FW_Request::POST('data/posts','[]');
			$posts = array_filter(json_decode($posts),'is_numeric');

			$items = array();

			switch ( $type ) {
				//@todo : test query
				case 'taxonomy_by_posts':
					if (empty($posts)){
						break;
					}
						...
						call_user_func_array(
							...
							array_merge(
								array(
									...
									"FROM $wpdb->terms as terms, $wpdb->term_taxonomy as taxonomies, $wpdb->term_relationships as relationship " .
									"WHERE terms.name LIKE %s AND taxonomies.taxonomy IN ( " .
									implode( ', ', array_fill( 1, count( $names ), '%s' ) ) .
									" ) " .
									"AND relationship.object_id IN (".
									implode( ', ', array_fill( 1, count( $posts ), '%s' ) ) .
									" ) " .
									"AND terms.term_id = taxonomies.term_id " .
									"AND relationship.term_taxonomy_id = taxonomies.term_taxonomy_id " .
									"AND taxonomies.term_id = taxonomies.term_taxonomy_id " .
									"LIMIT 100",
									'%' . $wpdb->esc_like( $title ) . '%'
								),
								array_merge($names,$posts)
							)
						)
					);
					break;
				case 'banner':
					...
					$items = $wpdb->get_results(
						call_user_func_array(
							...
							array_merge(
								array(
									"SELECT posts.ID val, posts.post_title title " .
									"FROM $wpdb->posts as posts " .
									"LEFT JOIN $wpdb->term_relationships t ON (posts.ID = t.object_id) " .
									"WHERE posts.post_title LIKE %s " .
									"AND t.term_taxonomy_id=$term " .
									"AND post_type IN ( " .
									implode( ', ', array_fill( 1, count( $names ), '%s' ) ) .
									" ) " .
									"LIMIT 100",
		...
		protected function _render( $id, $option, $data ) {
			...
			if ( isset( $option['population'] ) ) {
				switch ( $option['population'] ) {
					...
					case 'taxonomy_by_posts':
						//@todo: test query
						if ( isset( $option['source'] ) && isset($option['posts']) && !empty($option['posts']) && is_array($option['posts']) ) {
							/**
							 * @var WPDB $wpdb
							 */
							global $wpdb;
							$population = 'taxonomy_by_posts';
							...
							if ( isset( $option['prepopulate'] ) && ! empty( $source )
								...
								$terms = $wpdb->get_results(
									...
									"AND relationship.object_id IN ('" . implode("',", $option['posts']) . "') " .
									"AND relationship.term_taxonomy_id = taxonomies.term_taxonomy_id " .
									"AND terms.term_id = taxonomies.term_id " .
									"AND taxonomies.term_id = taxonomies.term_taxonomy_id "
								);
							...
							$query = $wpdb->get_results(
								...
								"FROM $wpdb->terms as terms, $wpdb->term_taxonomy as taxonomies, $wpdb->term_relationships as relationship " .
								"WHERE terms.term_id IN ( $ids )" .
								"AND taxonomies.taxonomy IN ( $in_sources ) " .
								"AND relationship.object_id IN ('" . implode("',", $option['posts']) . "')" .
								"AND relationship.term_taxonomy_id = taxonomies.term_taxonomy_id " .
								"AND terms.term_id = taxonomies.term_id " .
								"AND taxonomies.term_id = taxonomies.term_taxonomy_id "
						...
						break;
					case 'banner' :
						if ( isset( $option['source'] ) && isset($option['term']) && !empty($option['term']) ) {
							...
							if ( isset( $option['prepopulate'] )
								...
								$posts = $wpdb->get_results(
									...
									"AND t.term_taxonomy_id=" . $option['term'] . ' '.
									"AND post_status IN ( 'publish', 'private' ) " .
				...
				$option['attr']['data-posts']      =  isset($option['posts']) && !empty($option['posts']) && is_array($option['posts']) ?  json_encode($option['posts']) : '[]';
				$option['attr']['data-term']       = (isset($option['term']) && !empty($option['term']) && is_numeric($option['term'])) ? $option['term'] : '' ;
		...
		protected function _enqueue_static( $id, $option, $data ) {
			wp_enqueue_style(
				...
				fw()->extensions->get('offshore-ads-manager')->get_uri( '/includes/option-types/' . $this->get_type() . '/static/css/style.css' ),
				array( 'fw-selectize' ),
				fw()->manifest->get_version()
			);
			wp_enqueue_script(
				$this->get_type() . '-styles',
				fw()->extensions->get('offshore-ads-manager')->get_uri( '/includes/option-types/' . $this->get_type() . '/static/js/scripts.js' ),
	...
	FW_Option_Type::register( 'FW_Option_Type_Multi_Select_Extended' );

	add_action( 'wp_ajax_admin_action_ajax_response',
		array( "FW_Option_Type_Multi_Select_Extended", '_admin_action_get_ajax_response' ) );
